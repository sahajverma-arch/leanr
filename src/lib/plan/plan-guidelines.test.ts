...
function makeDay(dayIndex: number, mealsInput: Omit<PlanViewMeal, "id" | "archetypeId" | "archetypeName" | "archetypeDishFamilyIdsByExchangeType" | "note">[]): PlanViewDay {
  ...
  const meals: PlanViewMeal[] = mealsInput.map((m, i) => ({ ...m, id: `meal-${dayIndex}-${i}`, archetypeId: null, archetypeName: null, archetypeDishFamilyIdsByExchangeType: {}, note: null }))
